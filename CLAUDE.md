...
hierarchy: "Lord (human) → Shogun → Karo/Komadukai → Ashigaru 1-7 / Gunshi"
communication: "YAML files + inbox mailbox system (event-driven, NO polling)"

tmux_sessions:
  shogun: { pane_0: shogun }
  multiagent: { pane_0: karo, pane_1: komadukai, pane_2: gunshi, pane_3-9: ashigaru1-7 }
...
4. **Read your instructions file**: shogun→`instructions/shogun.md`, karo→`instructions/karo.md`, komadukai→`instructions/komadukai.md`, ashigaru→`instructions/ashigaru.md`, gunshi→`instructions/gunshi.md`. **NEVER SKIP** — even if a conversation summary exists. Summaries do NOT preserve persona, speech style, or forbidden actions.
...
## 2家老体制の運用ルール (cmd_173, 役割明確化 cmd_176)

### 役割分担

| 家老 | 役割 | 担当cmdの種類 |
|------|------|--------------|
| karo | **メインcmd統括** | プロジェクト改善・大型タスク・継続的な作業 |
| komadukai | **割り込みタスク担当** | 殿からの突発依頼・短期タスク・調査依頼 |

**assigned_karo の判断基準**（将軍がcmdを書く際）:
- 複数サブタスク・数日かかる → `assigned_karo: karo`
- 単発・短期・殿の直接依頼 → `assigned_karo: komadukai`
- 不明な場合 → `assigned_karo: karo`（デフォルト）

### 通信ルール

| 方向 | 手段 | 備考 |
|------|------|------|
| Shogun → karo/komadukai | assigned_karoで指定 | 各cmdのYAMLに担当家老記載 |
| karo ↔ komadukai | inbox_write | ダッシュボード・足軽競合調整 |
| karo/komadukai → 足軽 | 共有プール | 先に確保した方が使用 |
| karo/komadukai → gunshi | inbox_write | 両家老から依頼可 |

**ペイン番号（2家老体制）**:
karo=0.0, komadukai=0.1, gunshi=0.2, ashigaru1=0.3, ..., ashigaru7=0.9

**足軽共有プールルール**:
- 足軽(ashigaru1-7)は共有プール。空いている足軽を先に確保した方が使用。
- 競合時: inbox_writeで相互調整。
- dashboard.md: karo/komadukai 各自のセクションを更新。
